...
fn main() {
    ...
    let cmake_prefix_path = match env::var("TORCH_CMAKE_PREFIX_PATH") {
        Ok(path) => path,
        Err(_) => {
            let output = Command::new("python3")
                ...
                .output()
                .expect(
                    "torch-lib build error: failed to find Torch CMake prefix path \
                     (could not run python3 with `import torch`). \
                     Hint: install PyTorch or set TORCH_CMAKE_PREFIX_PATH.",
                );

            if !output.stderr.is_empty() {
                let stderr_str = String::from_utf8_lossy(&output.stderr);
                println!(
                    "cargo:warning=torch-lib: error getting torch prefix path: {}",
                    stderr_str.trim()
                );
            }
