...
The combined retained report passed child gates for both backends, commit consistency, and model provenance consistency. The soak JSON records first/last-quartile tail, throughput, RSS, and device-memory drift, but numeric drift is still reported evidence rather than a production budget.
