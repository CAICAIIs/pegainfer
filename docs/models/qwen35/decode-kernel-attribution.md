...
Neither invariant is covered by the HF gate above. `hf_golden_gate` replays fixed teacher-forced sequences and compares logprobs for the tokens it is given — it never selects a token, and the row logsumexp it scores against is mask-invariant (`exp(-inf) = 0`), so a missing mask is invisible to it by construction. The regression coverage for both invariants is in `pegainfer-sample`'s `select_batch` tests: `padded_arena_width_does_not_suppress_the_argmax_routing` places `top_p` exactly on the `1/decodable` boundary with a padded arena, and the suite's other tests pin argmax/sampler routing. Nothing in the suite would fail if the mask stopped being applied on the real model path, which is why the GEMM and the mask are one method rather than two statements at each site.
