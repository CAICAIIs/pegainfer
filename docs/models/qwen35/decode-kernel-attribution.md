...
1. **The split-KV partial kernel is latency-bound, and six structures have not moved it.** The largest single item at 0.4–0.6 ms/step. At c16 the shipped kernel runs 8192 warps of 32 positions at roughly 60 instructions a position, about 15.7M warp-instructions against 75.5M issue slots in the 124 µs it takes: a 21% issue utilisation, so it is waiting rather than issuing, and the load-only control says the same bytes can be read in 54 µs. Six FFMA structures have now been measured against the same reference — one position per lane, four positions per lane, the page-table hoist, shared-memory staging, staging with the proven per-position update, and software prefetching — and only the staging hybrid moved anything, by 16% at bs1 and −80% at c16. Tensor cores have also been ruled out. The next attempt should explain the 54 → 124 µs gap before writing another kernel: the load-only control issues the same loads from the same addresses and reaches 1235 GB/s, so the difference is in how the full kernel's loads interact, not in how many instructions follow them.
...
- **Software prefetching the next position.** Staging the next position's K before consuming the current one is correct and slower at every geometry tried: 40.0 µs against 37.7 at bs1, 133.9 against 124.3 at c16. The compiler's own scheduling plus four warps per scheduler already cover what one extra load in flight buys, and the extra registers and the guard cost more.
