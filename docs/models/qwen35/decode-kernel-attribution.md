...
1. **A tensor-core split-KV paged decode for HD256.** The largest single item at 0.59 ms/step. vLLM runs `flash_fwd_splitkv` and reaches 72 µs per layer-step against the 124 µs the register-streaming kernel costs and the 54 µs memory floor; the split-KV plumbing this kernel needs — partial buffers, the merge kernel, the batch threshold, the dispatch — is already in the tree from #1072, so the work is the partial kernel itself. Pack the batch into M (16 rows = 4 requests × 4 query heads share one K tile) and keep `mma.m16n8k16` for both QK and PV.
...
- **Staging K and V through shared memory in the tile form.** One global read per tile serves all four warps, and it is the fastest variant measured at bs1 by a wide margin (21.0 µs at 16 splits against 37.7 µs for the register form at 32). It was not shipped because it diverges from the reference kernel, and the divergence is not in the online update: correcting the running maximum to `exp(m_old - max(m_old, tile_m))` left the error bit-identical, and the error shrinks as the KV length grows (max_rel 33 at 32 positions, 17.9 at 64, 6.6 at 1024), which points at the tile-wide bookkeeping rather than at any single position. Treat the 21 µs as an upper bound that is not yet reachable, not as an available win.
