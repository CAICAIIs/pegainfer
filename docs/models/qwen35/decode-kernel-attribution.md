...
1. **More loads in flight in the split-KV partial kernel.** Prefetching both operands one position ahead took bs1 from 38.6 to 32.1 µs, and an eight-position `cp.async` double buffer took it to 30.0 µs at bs1 and 100.3 at c16 against 126.3 for the register form. The kernel is still short of its floor: the load-only control moves the same bytes from the same addresses in 54 µs at c16 against 100.3, and the remaining distance is now the per-position dependent chain in the compute rather than the loads. The next lever is instruction-level parallelism inside that chain, not more lookahead.
