...
1. **The split-KV partial kernel is latency-bound, and six structures have not moved it.** The largest single item at 0.4–0.6 ms/step. At c16 the shipped kernel runs 8192 warps of 32 positions at roughly 60 instructions a position, about 15.7M warp-instructions against 75.5M issue slots in the 124 µs it takes: a 21% issue utilisation at about 48 resident warps per SM, so it is waiting rather than issuing, and the load-only control says the same bytes can be read in 54 µs. Six FFMA structures have now been measured against the same reference — one position per lane, four positions per lane, the page-table hoist, shared-memory staging, staging with the proven per-position update, and software prefetching — and only the staging hybrid moved anything, by 16% at bs1 and −80% at c16. Tensor cores have also been ruled out. The next attempt should explain the 54 → 124 µs gap before writing another kernel: the load-only control issues the same loads from the same addresses and reaches 1235 GB/s, so the difference is in how the full kernel's dependent chain keeps those loads apart, and the register budget is a live suspect since the one attempt to overlap them spilled.
