...
That is −17% at bs1, −8% at bs4, and equal to −2% above, and it is a kernel-path measurement: at bs1 the eight full-attention layers cost 0.31 ms of an 8.57 ms step, so the serving effect is about 0.05 ms and sits at the edge of what this card's run-to-run spread resolves. bs1 mean TPOT read 8.51 / 8.51 / 8.54 ms after the change against 8.55–8.59 ms across the six runs before it.
...
1. **More loads in flight in the split-KV partial kernel.** Prefetching both operands one position ahead took bs1 from 38.6 to 32.1 µs and left the larger batches where they were, because one position of lookahead is one iteration of cover. The kernel is still latency-bound: at c16 it runs 8192 warps of 32 positions at roughly 60 instructions a position, about 15.7M warp-instructions against 75.5M issue slots in the 124 µs it takes, a 21% issue utilisation, while the load-only control moves the same bytes from the same addresses in 54 µs. Deeper lookahead needs the operand pairs out of registers and into a double-buffered shared-memory tile fed by `cp.async`, which is the one structure not yet tried and the one that matches the diagnosis.
