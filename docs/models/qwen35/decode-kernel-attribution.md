...
The per-step split of the two c16 captures says where the 1.6 ms/step difference sits, and no item in it is cheap. Per decode step: GEMM +0.72 ms (projection arrangement +0.13, the two M=1024 k/v GEMMs +0.15 which vLLM folds into its M=10240 q+k+v, the b/a pair +0.07, the split-K reduction +0.11, and prefill-shape differences +0.21), full-attention decode +0.59 ms, GDN prefill +0.25 ms, GDN decode +0.17 ms, with elementwise and activation running about 0.1 ms/step in our favour. Every remaining item is either a kernel that has to be rewritten or a refactor.

1. **Memory-level parallelism in the split-KV partial kernel, not fewer instructions.** The largest single item at 0.4–0.6 ms/step, and the record above says the direction. The kernel is at a 21% issue utilisation with 16 warps resident per SM, so it is waiting on one position's global load at a time; the cooperative tile form is 1.8x faster at bs1 and its defect is in the tile-wide bookkeeping that the hybrid preserved. Finish that variant rather than reaching for tensor cores. The split-KV plumbing — partial buffers, the merge kernel, the batch threshold, the dispatch — is already in the tree from #1072, so any replacement keeps the same entry point.
...
3. **The prefill GEMM shapes.** Our prefill runs three tile families (256x128, 128x256 and 128x128) where vLLM runs two, for +0.21 ms/step, and the prefill path is the untuned one: `gemm_lt_tune` covers only N ≤ 32, so every prefill GEMM takes cuBLAS's default heuristic. Two levers, neither of which has to touch a kernel: extend the timed plan to the prefill chunk size, and widen the candidate set, since the tuner only ever times the at most 16 algos `cublasLtMatmulAlgoGetHeuristic` returns while `cublasLtMatmulAlgoGetIds` plus `cublasLtMatmulAlgoInit`/`AlgoConfigSetAttribute` enumerates every tile, stage, split-K and swizzle the library can build. This is the same class of change as #1046. Watch the stream hazard: `launch_gemm` keeps cuBLASLt off any stream override, because two green-context streams running `cublasLtMatmul` concurrently has produced Xid 31.
...
- **Instruction count is not what limits the split kernel.** The rejection above follows from the arithmetic rather than from the two kernels. At c16 the shipped kernel runs 8192 warps of 32 positions each at roughly 60 instructions a position, about 15.7M warp-instructions against 75.5M issue slots in the 124 µs it takes — a 21% issue utilisation with only 16 warps resident per SM. Cutting the QK instructions therefore buys nothing: the kernel is waiting on memory, one position's global load at a time, and what helps is memory-level parallelism. That is the same explanation for the two results that did move: cooperative shared-memory staging of a 64-position tile is 1.8x faster at bs1 (21.0 µs), and the correctness-preserving hybrid that keeps the per-position loop is 80% slower at c16 (223.7 µs) because the staging is followed by the same one-position-at-a-time chain.
- **A shared-memory tile form with a faster structure.** One global read per tile serves all four warps and it is the fastest variant measured at bs1 by a wide margin (21.0 µs at 16 splits against 37.7 for the register form at 32). It was not shipped because it diverges from the reference kernel, and the divergence is not the online update: correcting the running maximum to `exp(m_old - max(m_old, tile_m))` left the error bit-identical, and the error shrinks as the KV length grows (max_rel 33 at 32 positions, 17.9 at 64, 6.6 at 1024), which points at the tile-wide bookkeeping rather than at any single position. Treat the 21 µs as an upper bound that is not yet reachable, not as an available win.
- **Shared-memory staging with the proven per-position update.** The combination is correct (`max_rel` 6.5e-3, matching the register form) and lands at 31.5 µs at bs1, but it is 80% slower at c16 (223.7 µs against 124.3) because the tile load and the per-position loop serialise behind each barrier. The staging cannot be had without the tile-wide form.
