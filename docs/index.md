...
| `models/glm52/free-running-dp.md` | 设计文档:删除 DP coordinator——每个 DP rank 是独立 engine(自己的 scheduler/BlockPool/HTTP endpoint),loop 无条件全速跑,唯一耦合是固定节拍的 DeepEP collective 链;三条纪律(固定链/保守 bound/padding 即协议)。Go/no-go gates 1-3 已实现(`oracle/freerun_ep4.rs`),待 NVL72 出数。取代 cross-node-scaling.md 的 Event plane 与 SMR 方向。 |
