...
| `benchmarks/qwen35-4b-serving-vllm027-a100-2026-09.md` | Gap map vs latest vLLM 0.27.0 on 1x A100-40GB (single run per cell): zero failures; PegaInfer wins 1024-token bs1 TTFT `78 vs 118ms` but loses batch TPOT +24-35%, ITL p99 at c8 `65 vs 12ms` (prefill invades decode), and QPS16 TTFT `867 vs 218ms`; bs1 decode TPOT grows with context on ours, flat on vLLM. |
