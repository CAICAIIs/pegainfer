...
| `models/glm52/free-running-dp.md` | 设计文档:删除 DP coordinator——每个 DP rank 是独立 engine(scheduler/BlockPool/HTTP endpoint 自持),loop 无条件全速跑,唯一耦合是固定节拍的 DeepEP collective 链;三条纪律(固定链/保守 bound/padding 即协议)。**Gate 1–3 已在 GB300 tray03 全 GO(2026-07-30):流量不变性与异构 graph 回放 bit-exact,保守 bound 税=0。** 取代 cross-node-scaling.md 的 Event plane 与 SMR 方向。 |
