...
| `models/glm52/free-running-dp.md` | 设计文档:删除 DP coordinator——每个 DP rank 是独立 engine(scheduler/BlockPool/HTTP endpoint 自持),loop 无条件全速跑,唯一耦合是固定节拍的 DeepEP collective 链;三条纪律(固定链/保守 bound/padding 即协议)。**Gate 1–3 全 GO(GB300 tray03);迁移第 1 步已实装(per-rank bucket、协议最大 bound、MTP 固定链),gate 4/5 待 GPU 实测。** 取代 cross-node-scaling.md 的 Event plane 与 SMR 方向。 |
